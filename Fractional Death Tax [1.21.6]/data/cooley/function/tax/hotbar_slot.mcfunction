...
$execute if score #shulker_whitelist co_math matches 0 if score #tax_shulker co_math matches 1 if items entity @s hotbar.$(hotbar_slot) #minecraft:shulker_boxes run function cooley:tax/container/shulker_for {setup:"Inventory[{Slot:$(shift_slot)b}]",replace:"hotbar.$(hotbar_slot)"}
